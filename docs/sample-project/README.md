...
```bash
bunx pm2 start dist/index.js --name taskflow-api
pm2 startup
pm2 save
```
...
| Script | Descripción |
|--------|-------------|
| `bun run dev` | Inicia en modo desarrollo con hot-reload |
| `bun run build` | Compila TypeScript a JavaScript |
| `bun start` | Inicia el servidor compilado |
| `bun test` | Corre todos los tests |
| `bun run test:watch` | Tests en modo watch |
| `bun run db:migrate` | Aplica migraciones pendientes |
| `bun run db:seed` | Carga datos de ejemplo |
| `bun run db:reset` | Resetea la base de datos (solo desarrollo) |
...
4. Asegúrate que los tests pasen: `bun test`
