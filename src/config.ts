...
function validateProvider(provider: string): ModelProvider {
    ...
    throw new Error("MODEL_PROVIDER invalido:" + provider + "Debe ser anthropic o openai");
}
...
export function validateConfig(): void {
    if (config.provider === "anthropic" && !config.anthropicApiKey){
        throw new Error("ANTHROPIC_API_KEY esta vacia. agregala en tu archivo .env");
    }
    if (config.provider === "openai" && !config.openaiApiKey){
        throw new Error("OPENAI_API_KEY esta vacia. agregala en tu archivo .env");
    }
}
